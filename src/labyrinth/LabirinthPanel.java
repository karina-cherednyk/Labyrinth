...
public class LabirinthPanel extends JPanel {
    ...
    LabirinthPanel(Tools t) {
        ...
        this.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                switch (code) {
                    case KeyEvent.VK_UP:
                        if (y - 1 >= 0 && blocks[y - 1][x] != false) {
                            y--;
                        }
                        break;
                    case KeyEvent.VK_DOWN:
                        if (y + 1 < blocks.length && blocks[y + 1][x] != false) {
                            y++;
                        }
                        break;
                    case KeyEvent.VK_LEFT:
                        if (x - 1 >= 0 && blocks[y][x - 1] != false) {
                            x--;
                        }
                        break;
                    case KeyEvent.VK_RIGHT:
                        if (x + 1 < blocks[1].length && blocks[y][x + 1] != false) {
                            x++;
                        }
                        break;
                    default:
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < blocks.length; i++) {
            for (int j = 0; j < blocks[i].length; j++) {
                if ((i - yHovered) >= 0 && (i - yHovered) < 3) {
                    ...
                    } else if (j == xHovered + 3) {
                        g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1));
                    }
                }

                if (blocks[i][j] == false) {
                    g2.drawImage(canNotGo, j * size, i * size, null);
                } else if (blocks[i][j] == true) {
                    g2.drawImage(canGo, j * size, i * size, null);
                }
                if (i + 1 == blocks.length && j % 3 == 0) {
                    g2.drawLine(j * size, 0, j * size, getHeight());
                }
            }
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1));
            if (i % 3 == 0) {
                g2.drawLine(0, i * size, getWidth(), i * size);
            }
        }
        if (tools.darkness) {
        ...
        for (int i = 0; i < coinLocation.length; i++) {
            ...
            if (coinLocation[i] == null) {
                continue;
            } else if (x == coinLocation[i][0] && y == coinLocation[i][1]) {
                coinLocation[i] = null;
                 tools.main.g.coins.setFramePosition(0);
                tools.main.g.coins.start();
                tools.setCoins(tools.coins += 1);
                tools.main.g.coinsLab.setText("COINS: " + tools.coins);
            } else {
                g2.drawImage(coin, coinLocation[i][0] * size, coinLocation[i][1] * size, null);
            }
        }
